...
20) (optional) Choose an offset transformation
Each placed mesh will be offset from its usual position with the chosen transformation.  This can be used to offset the meshes from the spline, e.g. to place details on the side of the main splinemesh sequence.
...
Note that rotational offsets only apply to static meshes, not to parts of the splinemesh sequence.
...
22) (optional) Add Spline Up Vector hints
Sometimes, the engine cannot determine which direction is "up" from the spline, which can lead to unwanted twisting, especially around sharp corners or when trying to make loop-do-loop effects.
Entries can be added to the "Spline Up Vectors" section to give hints for the up vector direction for different parts of the spline.
Each hint consists of a distance along the spline, and then a vector.  That vector will be applied to any spline meshes placed after the specified distance.
A pink arrow will be shown in the editor at the points where the up vector changes.


22) (optional) Configure Material Overrides
The materials used on the meshes inside the sequence can be overridden. This can be configured by specifying an original material to replace, and then one or more replacement materials.
Any meshes using the specified original material will have it replaced by one of the replacement materials, selected from the array in sequence.
...
Performance
===================
Generally, the spline sequence should only take moments to build each time the spline is adjusted.  However, the engine may struggle with very complex meshes.  Also, note that Unreal does not support Nanite on spline meshes.
If you are creating a mesh sequence using a very complicated mesh, it may be advisable to use a lower-poly version (or a lower LOD) while placing the sequence, and then switching to the high-poly mesh once you are happy with the placement.  You may also want to merge the spline mesh sequence into a static mesh (see section below.)

Another potential cause of performance issues is a slow Derived Data Cache.  The Unreal documentation suggests placing a Derived Data Cache in a shared location where all team members can access it, but if that is a slow network location, then it can cause significant slowdowns in certain operations, including updating spline meshes.




Merging spline sequences to static meshes:
===================
Spline meshes are somewhat limited compared to normal static meshes.  Some engine features, such as Naniite, do not work with them, and they may have worse rendering performance.
As such, you may wish to convert a completed mesh sequence to a static mesh once you are satisfied with its placement.
To do this, simply right-click on the spline mesh sequence object in the editor and select Merge Actors.  The default settings are generally fine.

Note that if your sequence contains static meshes along the spline, you will need to change their Static Mesh Type to Static before merging, as Unreal Engine has issues with merging the instanced static meshes which NVSplineTools uses by default.
...
v9.1
	- Added support for stard and end meshes for "Periodic", "At Points", and "Stretched Between Points" placement methods.

v9.0
	- Added support for material replacement.

v8.1
	- Fixed issue with divide-by-zero error when not using Twist Count, leading to the log being spammed with warnings.

v8.0
	- Added support for offset transform and random offset transforms for spline meshes.

v7.0
	- Added the ability to set the spline up direction at specified distances.
